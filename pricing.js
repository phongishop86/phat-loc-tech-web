const PricingFlatWidget = {
    ...
    renderSkeleton() {
        this.container.innerHTML = `
            <div class="py-4 bg-slate-50/50" id="pricing-table">
                <div class="container mx-auto px-4 max-w-6xl">
                    <div class="text-center mb-6">
                        <p class="text-slate-500">Đang tải dữ liệu từ máy chủ...</p>
    ...
    render() {
        ...
        const tableRows = displayItems.length > 0 ? displayItems.map(item => `
            <tr class="hover:bg-slate-50 transition-colors border-b border-slate-200 last:border-0 group flex flex-col sm:table-row p-3 sm:p-0">
                <td class="p-2 sm:p-4 text-slate-800 group-hover:text-primary transition-colors block sm:table-cell font-semibold sm:font-normal text-base sm:text-sm">
                    ${item.name}
                    <div class="text-xs text-slate-400 mt-1 sm:hidden">${item.category}</div>
                </td>
                <td class="hidden lg:table-cell p-2 sm:p-4 text-left sm:text-center text-slate-400 text-xs">
                    <span class="bg-slate-100 px-2 py-1 rounded-md">${item.category}</span>
                </td>
                <td class="p-2 sm:p-4 text-left sm:text-center text-slate-400 flex sm:table-cell justify-between items-center text-sm border-t border-slate-200 sm:border-0 mt-2 sm:mt-0 pt-2 sm:pt-4">
                    <span class="sm:hidden font-semibold text-slate-500 text-xs">Bảo hành:</span>
                ...
                <td class="p-2 sm:p-4 text-left sm:text-right font-medium text-brand-green flex sm:table-cell justify-between items-center text-base sm:text-sm">
                    <span class="sm:hidden font-semibold text-slate-500 text-xs">Đơn giá:</span>
                ...
                <td class="p-2 sm:p-4 text-right sm:text-center block sm:table-cell mt-3 sm:mt-0">
                    <button data-action="add-cart" data-name="${item.name.replace(/"/g, '&quot;')}" data-price="${item.price}" data-warranty="${item.warranty}" class="w-full sm:w-auto text-slate-700 sm:text-slate-400 hover:text-primary sm:hover:text-brand-green bg-brand-green/20 hover:bg-brand-green/40 sm:bg-white sm:hover:bg-slate-100 p-2.5 sm:p-2 rounded-lg transition-colors border border-brand-green/30 sm:border-slate-200 hover:border-brand-green sm:hover:border-brand-green/50 group-hover:shadow-[0_0_15px_rgba(16,185,129,0.2)] flex items-center justify-center gap-2" title="Thêm vào giỏ hàng">
        ...
        `).join('') : `<tr><td colspan="5" class="p-8 text-center text-slate-400">Không tìm thấy sản phẩm nào phù hợp.</td></tr>`;

        this.container.innerHTML = `
            <div class="py-4 bg-slate-50/50" id="pricing-table">
                <div class="container mx-auto px-4 max-w-7xl">
                    <div class="w-full">
                        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-lg shadow-slate-200/50 mb-6 lg:mb-8 shadow-xl relative overflow-hidden">
                            ...
                            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
                                <div>
                                    ...
                                    <p class="text-sm text-slate-500">${showingText}</p>
                                </div>
                                <div class="relative w-full md:w-80 shrink-0">
                                    <input type="text" 
                                        placeholder="Tìm kiếm ${this.flatItems.length} sản phẩm..." 
                                        class="w-full bg-slate-50 border border-slate-300 text-primary text-sm rounded-xl pl-10 pr-4 py-3 focus:outline-none focus:border-brand-green focus:ring-1 focus:ring-brand-green transition-all"
                        ...
                        <div class="bg-white/90 shadow-xl shadow-slate-200/50 border border-slate-200 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-sm">
                            <div class="overflow-x-auto">
                                <table class="w-full text-left border-collapse min-w-[300px]">
                                    <thead>
                                        <tr class="bg-white/80 backdrop-blur-md text-slate-500 text-xs uppercase tracking-wider hidden sm:table-row border-b border-slate-200">
